...
import { FORMATIONS, DIVISIONS, getDivisionFieldHeight, getFormationsForDivision } from '../formations'
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (FORMATIONS[formation]) {
      setLineupPositions(
        FORMATIONS[formation].positions.map(pos => ({ ...pos, playerId: undefined }))
      )
    }
  }, [formation, division])
  ...
  const handleAddPlayer = (teamId: string, playerName: string, playerNumber: number) => {
  ...
  const handleDeletePlayer = (teamId: string, playerId: string) => {
    ...
    setLineupPositions(lineupPositions.map(pos => 
  ...
  const handleResetLineup = () => {
    ...
    setLineupPositions(lineupPositions.map(pos => ({ ...pos, playerId: undefined })))
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-gray-900">Soccer Lineup Organizer</h1>
                <p className="text-sm text-gray-500">Create and manage your team lineups</p>
              </div>
            </div>
            
            {/* TODO: AUTHENTICATION POINT #4 - Add user menu and logout */}
        ...
        {activeTab === 'lineup' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-1 space-y-6">
              ...
              {/* Division Selection */}
              <div className="bg-white rounded-lg shadow-sm p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Division</h3>
                ...
                <div className="space-y-2">
                  {getFormationsForDivision(division).map(formKey => (
                    ...
                    >
                      ...
                      <div className={`text-sm ${formation === formKey ? 'text-green-100' : 'text-gray-500'}`}>
                        {FORMATIONS[formKey].name.split('(')[1]?.replace(')', '')}
                      </div>
                    </button>
                  ))}
                </div>
              ...
              <div className="bg-white rounded-lg shadow-sm p-6">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-lg font-semibold text-gray-900">
                    {selectedTeam ? selectedTeam.name : 'Select a team to begin'}
                  </h3>
